...
export default function DeleteModal({ closeModal, dataDelete, isOpen }) {
  ...
  const { mutate, isLoading, reset } = useMutation({
    mutationFn: async () => MappedAreasApi.Delete(dataDelete),
    onSuccess: () => {
      queryClient.refetchQueries(['AreasMapsAll'])
      closeModal()
      toast.success('Área mapeada excluído com sucesso')
      reset()
    },
    onError: () => {
      closeModal()

      toast.error('Não foi possível excluir a área mapeada')
      reset()
    },
  })
    ...
    >
      <div className="my-8 w-full pl-4">
        <p className="text-sm text-gray-500">
          Você tem certeza que deseja excluir a área mapeada?{' '}
